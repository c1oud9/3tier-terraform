...
# Azure MySQL Flexible Server 모듈
module "azure_mysql" {
...
# Route53 DNS Failover (Health Check 포함)
...
resource "aws_lambda_function" "db_sync" {
  filename         = "${path.module}/scripts/lambda-db-sync.zip"
  function_name    = "db-sync-${var.environment}"
  role            = aws_iam_role.lambda_sync.arn
  handler         = "index.handler"
  source_code_hash = filebase64sha256("${path.module}/scripts/lambda-db-sync.zip")
  runtime         = "python3.11"
  timeout         = 300
  memory_size     = 512
...
resource "aws_cloudwatch_event_rule" "db_sync_schedule" {
  ...
  description         = "Trigger DB sync every 5 minutes"
...
resource "aws_cloudwatch_dashboard" "main" {
  dashboard_name = "multi-cloud-dr-${var.environment}"
